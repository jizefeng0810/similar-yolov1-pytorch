...
class Dataset_yolo(data.Dataset):
    ...
    def encoder(self, boxes, labels):
        ...
        return nxnxn
        '''
        grid_num = Config.cell_size
        target = torch.zeros((grid_num, grid_num, 5+Config.num_class))
        target[:,:,4] = 1       # backgroun set one
        cell_size = 1. / grid_num
        wh = boxes[:, 2:] - boxes[:, :2]
        cxcy = (boxes[:, 2:] + boxes[:, :2]) / 2.0
        for i in range(cxcy.size()[0]):
            ij = (cxcy[i] / cell_size).ceil() - 1  #
            target[int(ij[1]), int(ij[0]), 4] = 0
            target[int(ij[1]), int(ij[0]), 4+Config.num_class] = 1
            target[int(ij[1]), int(ij[0]), 2:4] = wh[i]
            target[int(ij[1]), int(ij[0]), :2] = cxcy[i]
        return target
...
file_root = '.' + Config.train_image_path

def main():
    from torch.utils.data import DataLoader
    import torchvision.transforms as transforms
    train_dataset = Dataset_yolo(image_root=file_root, list_file=train_txt_path, train=False,
                                transform=[transforms.ToTensor()])
